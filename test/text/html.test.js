...
test("文本-注音", async () => {
	...
	input = "![文本-注音]";
	expect(await showTextBrowser(input)).toBe("![文本-注音]");
});
...
test("文本-文字颜色", async () => {
	...
	input = "![文本-文字颜色]";
	expect(await showTextBrowser(input)).toBe("![文本-文字颜色]");
});

test("文本-黑幕", async () => {
	let input = "![文本-黑幕](汉字)";
	expect(await showTextBrowser(input)).toBe(
		'<span class="heimu">汉字</span>'
	);
	...
	input = "![文本-黑幕]";
	expect(await showTextBrowser(input)).toBe("![文本-黑幕]");
});
